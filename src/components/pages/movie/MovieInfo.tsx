...
const MovieInfo = memo(({ detailData, castList, trailerData, castLoading, videoLoading }: MovieInfoProps) => {
  return (
      ...
      <ul className="grid sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-5 pb-10 border-b border-solid border-muted md:gap-8">
        {infoList.map((item) => (
          <li key={item.title}>
            ...
            {item.title === 'Homepage' && detailData?.[item.value] && String(detailData?.[item.value])?.trim() ? (
